...
4. **Suavização de Imagem (Blur por Média)**
   - A rede neural do CoProcessador foi treinada com imagens do dataset MNIST, onde os dígitos são naturalmente suaves e com bordas borradas. Um desenho feito com o mouse, por outro lado, é formado por blocos de pixels completamente brancos (255) ou completamente pretos (0), sem transições graduais. Pra aproximar a distribuição da imagem desenhada à que o modelo espera, a Aplicação pode aplicar um filtro de blur por média simples de janela 3×3, seguido de normalização pelo valor de pico, antes de enviar a imagem ao CoProcessador.
